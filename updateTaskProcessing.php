    ...
    */
    ...
    if ($error == 1)
    {
    	...
    	$_SESSION['uEmpId']= (empty($_POST['emp'])?"":$_POST['emp']);
     	...
     	$sql= "SELECT status, priority, description, end_date FROM tasks WHERE uid like '".$_POST['taskId']."'";
     	$result= $db->query($sql);
	 	...
	 	*/
     	$employee = (empty($_POST['emp'])?"":$_POST['emp']);
     	$date = DateTime::createFromFormat('m/d/Y', $holdED);
    	$holdED = $date->format('Y-m-d');
   		 if ($_POST['status'] != $row[0] || $_POST['priority'] != $row[1] || $holdDesc != $row[2] || $holdED != $row[3] || $employee)
     	{
     	if ($_POST['status'] != $row[0] && $_POST['status'] == 'Completion Approved' )
     	{
     		$date = DateTime::createFromFormat('m/d/Y', $_POST['clientTime']);
    		$holdED = $date->format('Y-m-d');
     	}
