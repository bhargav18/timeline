...
if (!empty($_POST))
    if ($_POST['create']) {
        ...
        $ifuser = $db->query('select role from users where email="' . $email . '" and role="'.$role.'"');
        error_log('select role from users where email="' . $email . '" and role="'.$role.'"');
        if (mysqli_num_rows($ifuser) > 0 ) {
            error_log("Here");
