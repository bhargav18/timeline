...
class tasks {
    ...
    function getTasks_JSON($user_uid, $project_uid) {
        ...
        if ($_SESSION['access_level'] == 2) {
            ...
            $result = $this->db->query('select * from tasks where uid in(select distinct(t7.uid) from (select t1.*,t2.last_name,t2.uid as user_uid,t3.status as project_st from tasks t1, project t3, users t2 where t1.uid in (select task_uid from user_tasks where user_uid='.$user_uid.') and (t1.project_uid=t3.uid or ISNULL(t1.project_uid)) and t1.deleted="N" and t2.uid='.$user_uid.') as t7)');
        }
...
';
        foreach ($rows as $row) {
            ...
            $json_data.='"description":"' . $row['last_name'] . ''
                    ...
                    . '<br/><span>Comment to this task:</span><br/><textarea id=\"reply_box\" style=\"height:40px;width:300px;\"></textarea><input type=\"button\" value=\"Comment\" onclick=\"reply_this('.$row[uid].')\"/>' . '",';
